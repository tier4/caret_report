...
# Callback
python3 "${script_path}"/callback/validate_callback.py "${trace_data}" --component_list_json="${component_list_json}" --expectation_csv_filename="${expectation_callback_csv_filename}" -s "${start_time}" -d "${duration_time}" -f -v
python3 "${script_path}"/callback/make_report_callback.py "${report_dir_name}" --component_list_json="${component_list_json}"

# Topic
python3 "${script_path}"/topic/validate_topic.py "${trace_data}" --component_list_json="${component_list_json}" --expectation_csv_filename="${expectation_topic_csv_filename}" -s "${start_time}" -d "${duration_time}" -f -v
