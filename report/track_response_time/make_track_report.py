...
value_name_list = ['best_p99',  'best_p95', 'best_avg', 'best_p50']
value_name_display_list = ['99%ile', '95%ile', 'avg', 'median']
color_list = ['red', 'blue', 'green', 'orange', 'purple', 'black', 'brown', 'pink', 'gray', 'olive', 'cyan']
...
def render_page(version_list, stats_per_path, filename_per_path, destination_path, template_path):
    """Render html page"""
    with app.app_context():
        with open(template_path, 'r', encoding='utf-8') as f_html:
            ...
            rendered = flask.render_template_string(
                ...
                stats_per_path=stats_per_path,
                filename_per_path=filename_per_path,
            )
    ...
    with open(stats_list_file, 'r', encoding='utf-8') as f_csv:
        for row in csv.DictReader(f_csv, ['version', 'stats_file']):
            ...
            if os.path.isfile(stats_file):
                ...
                stats_file_list.append((row['version'], stats_file))
            else:
                # path from csv file dir
                stats_file = os.path.dirname(stats_list_file) + '/' + stats_file
                if os.path.isfile(stats_file):
                    stats_file_list.append((row['version'], stats_file))
                else:
...
def make_stats(dest_dir: str, stats_list_file: str):
    ...
    for version, stats_file in stats_file_list:
        with open(stats_file, 'r', encoding='utf-8') as f_yaml:
            stats_per_version[version] = yaml.safe_load(f_yaml)

    target_path_name_list = [target_path_info['target_path_name'] for target_path_info in stats_per_version[stats_file_list[0][0]]]
        ...
        for target_path_name in target_path_name_list:
            for target_path_info in stats:
                if target_path_info['target_path_name'] == target_path_name:
                    df_per_path.loc[version][target_path_name] = [target_path_info[val] if type(target_path_info[val]) is float else np.nan for val in value_name_list]
                    break
    ...
    version_list = [version for version, stats in stats_per_version.items()]
    filename_per_path = {}
    stats_per_path = {}
    for target_path_name in target_path_name_list:
        stats = {}
        df = df_per_path[target_path_name]
        for index, value_name in enumerate(value_name_list):
            stats[value_name_display_list[index]] = {version: df[value_name][i] for i, version in enumerate(version_list)}
        stats_per_path[target_path_name] = stats
        filename_per_path[target_path_name] = make_file_name(target_path_name, dest_dir, False)

    return version_list, stats_per_path, filename_per_path
...
def make_report(dest_dir: str, stats_list_file: str):
    ...
    if ret is None:
        return
    version_list, stats_per_path, filename_per_path = ret
    destination_path = f'{dest_dir}/index.html'
    template_path = f'{Path(__file__).resolve().parent}/template.html'
    render_page(version_list, stats_per_path, filename_per_path, destination_path, template_path)
...
def main():
    ...
    dest_dir = args.report_directory[0] + '/track'
